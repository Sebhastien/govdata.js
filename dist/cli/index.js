...
program.name("govdata").description("CLI for accessing U.S. government data systems").version("1.0.0");
program.command("fpds").description("Search FPDS contracts").option("-c, --contract <contracts...>", "Contract number(s) (PIID). Can specify multiple contracts.").option("-d, --date-range <date-range>", "Date range [YYYY/MM/DD, YYYY/MM/DD]").option("-o, --output <file>", "Output file (.csv or .json). If not specified, outputs JSON to console.").option("--threads <count>", "Number of concurrent threads", "10").option("--naics <naics>", "NAICS code filter").option("--psc <psc>", "PSC code filter").option("--agency <agency>", "Contracting agency filter").option("--vendor <vendor>", "Vendor name filter").action(async (options) => {
  try {
    const params = {};
    if (options.contract) {
...
program.command("examples").description("Show usage examples").action(() => {
  console.log(`
